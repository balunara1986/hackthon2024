...
def prepare_dataset() -> None:
    ...
    human = os.path.join(root_dir, 'data/human/')
    ai = os.path.join(root_dir, 'data/ai/')
    train, test = create_splits(human, ai)

    voice_train = os.path.join(root_dir, 'data/plots/train/human/')
    ai_voice_train = os.path.join(root_dir, 'data/plots/train/ai/')
    voice_test = os.path.join(root_dir, 'data/plots/test/human/')
    ai_voice_test = os.path.join(root_dir, 'data/plots/test/ai/')

    create_dir(voice_train)
    create_dir(ai_voice_train)
    create_dir(voice_test)
    create_dir(ai_voice_test)
    ...
    for file in train:
        try:
            ...
            if 'ai' in file:
                ...
                jpg_file_name = str(ai_voice_train + jpg_file_name)
    ...
    for file in test:
        try:
            ...
            if 'ai' in file:
                ...
                # construct new jpg file name with the extenstion
                jpg_file_name = str(wav_name[0]) + '.jpg'
                jpg_file_name = str(ai_voice_test + jpg_file_name)
                get_melss(file, jpg_file_name)
            else:
                wav_name = os.path.basename(file)
                wav_name = os.path.splitext(wav_name)
