...
def apply_transforms(wav_file: str) -> dict:
    ...
    melss = np.mean(librosa.feature.melspectrogram(y=x, sr=sr).T, axis=0)
    ...
    return {'mfccs': mfccs, 'chromagram': chromagram, 'melss': melss, 'spec_contrast': spec_contrast,
            'tonnetz': tonnetz}
...
def transforms_to_tensor(transforms: dict) -> list:
    ...
    return torch.FloatTensor(transforms_list).to(device='cpu')
...
def prepare_dataset() -> None:
    annotations_path = os.path.join(root_dir, 'data/annotations')
    voice_wavs = str(os.path.join(root_dir, 'data/human/wav/'))
    ai_voice_wavs = str(os.path.join(root_dir,'data/ai/wav/'))

    print('Creating splits ...\n')
    train, test = create_splits(voice_wavs, ai_voice_wavs)
